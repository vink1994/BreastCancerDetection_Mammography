...
import sys
sys.path.append('G:/AFIM/init_lib/')
...
class AFIM_Train_Class():
    ...
    def test(self, test_loader):
        ...
        y_pred_loaded = torch.load('/V6/AFIM/weights/y_pred.pt')
        y_true_loaded = torch.load('/weights/y_true.pt')

        if self.num_classes == 1:
            auc = roc_auc_score(y_true_loaded , y_pred_loaded)
            print('AUC %s on the CBIS dataset for double views is: %.3f' % (self.net.__class__.__name__, auc))
